...
// GetPendingTasks returns slice of task.Signatures waiting in the queue
func (b *Broker) GetPendingTasks(queue string) ([]*tasks.Signature, error) {
	return nil, errors.New("not implemented")
}

// GetDelayedTasks returns slice of task.Signatures waiting in the queue
...
func (b *Broker) StopConsuming() {
	...
	case b.retryStopChan <- 1:
		log.Logger.Info("Stopping retry closure.")
	default:
	}
	// Notifying the stop channel stops consuming of messages
	close(b.stopChan)
	log.Logger.Info("Stop channel")
}
