...
// ErrTaskPanic ...
var ErrTaskPanic = errors.New("invoking task caused a panic")
...
func (t *Task) Call() (taskResults []*TaskResult, err error) {
	...
	defer func() {
		...
		if e := recover(); e != nil {
			...
			// Print stack trace
			log.Logger.Error("%s", debug.Stack())
		}
	}()
	...
	// to the caller.
	if !lastResult.IsNil() {
		// If the result implements Retryable interface, return instance of Retryable
		retryableErrorInterface := reflect.TypeOf((*Retryable)(nil)).Elem()
		if lastResult.Type().Implements(retryableErrorInterface) {
		...
		if !lastResult.Type().Implements(errorInterface) {
			return nil, ErrLastReutrnValueMustBeError
		}
